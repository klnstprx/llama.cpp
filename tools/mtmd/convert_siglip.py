...
def get_tensor_name(name: str) -> str:
    ...
    return (
        ...
        .replace("mlp.fc1", "ffn_up")
        .replace("mlp.fc2", "ffn_down")
...
encoder_group.add_argument(
    ...
    action="store_true",
    required=False,
    help="The clip model is from OpenCLIP (vision-only variant, e.g. ViT-SO400M)",
)
encoder_group.add_argument(
    "--clip-model-is-siglip",
    action="store_true",
    required=False,
    help="SigLIP vision-only encoder (no text tower)",
)
encoder_group.add_argument(
    "--clip-model-is-siglip-full",
    action="store_true",
    required=False,
    help="SigLIP dual-tower encoder containing both vision and text sub-models",
)
...
if args.text_only and args.vision_only:
    print(
        "--text-only and --vision-only arguments cannot be specified at the same time."
    )
    exit(1)
...
if (
    ...
    or not os.path.exists(os.path.join(dir_model, "vocab.json"))
    or args.clip_model_is_openclip
    or (args.clip_model_is_siglip and not args.clip_model_is_siglip_full)
):
    vocab = None
    tokens = None
else:
    with open(os.path.join(dir_model, "vocab.json"), "r", encoding="utf-8") as f:
        vocab = json.load(f)
        tokens = [key for key in vocab]

# Fallback: try to obtain tokens via transformers tokenizer when we still need them
if tokens is None and args.clip_model_is_siglip_full:
    try:
        ...
        print(
            f"WARNING: Failed to load tokenizer vocabulary automatically: {e}\n"
            "         Text encoder will be skipped."
        )
        tokens = None
        vocab = None
...
if args.clip_model_is_siglip and not args.clip_model_is_siglip_full:
    # Vision-only SigLIP encoder
    model = SiglipVisionModel.from_pretrained(dir_model)
    processor = None
elif args.clip_model_is_siglip_full:
    # Dual-tower SigLIP model (vision + text)
...
# we will add the feature flags later, once their final values are known
...
if args.text_only:
    ...
    # add projector type for llava
    fout.add_string("clip.projector_type", args.projector_type)
else:
    # dual tower without projector (e.g., SigLIP)
    fout.add_description("two-tower CLIP model")
    fout.add_string("clip.projector_type", "none")
if has_text_encoder and tokens is None:
    # We expected a vocab but failed to obtain it; fall back to disabling text encoder
    print("WARNING: tokens list is empty – disabling text encoder output in GGUF")
    has_text_encoder = False

if has_text_encoder:
    assert t_hparams is not None, (
        "Text hyperparameters missing while text encoder requested"
    )
    assert tokens is not None, "Token vocabulary missing while text encoder requested"
    if args.clip_model_is_siglip or args.clip_model_is_siglip_full:
        text_projection_dim = 0
    else:
        text_projection_dim = t_hparams.get("projection_dim", config["projection_dim"])

    # text_model hparams
    # Some SigLIP (and other) configs use different field names.
    def tparam(primary_keys, default=None):
        if isinstance(primary_keys, str):
            primary_keys = [primary_keys]
        for key in primary_keys:
            if t_hparams is not None and key in t_hparams:
                return t_hparams[key]
        if default is not None:
            return default
        raise KeyError(primary_keys[0])

    fout.add_uint32(
        k(KEY_CONTEXT_LENGTH, TEXT),
        tparam(
            [
    ...
    fout.add_uint32(k(KEY_EMBEDDING_LENGTH, TEXT), tparam(["hidden_size", "d_model"]))
    fout.add_uint32(
        k(KEY_FEED_FORWARD_LENGTH, TEXT),
        tparam(
            ["intermediate_size", "mlp_ratio"],
            default=tparam(["hidden_size", "d_model"]) * 4,
        ),
    )
    fout.add_uint32("clip.text.projection_dim", text_projection_dim)
    fout.add_uint32(
        k(KEY_ATTENTION_HEAD_COUNT, TEXT), tparam(["num_attention_heads", "n_heads"])
    )
    fout.add_float32(
        k(KEY_ATTENTION_LAYERNORM_EPS, TEXT), tparam(["layer_norm_eps"], default=1e-5)
    )
    fout.add_uint32(k(KEY_BLOCK_COUNT, TEXT), tparam(["num_hidden_layers", "n_layers"]))
...
if has_vision_encoder:
    ...
    # Helper for vision hyperparameters (handles alternative keys / nested fields)
    def vparam(primary_keys, *fallback_keys, default=None):
        if isinstance(primary_keys, str):
            primary_keys = [primary_keys] + list(fallback_keys)
        else:
            primary_keys = list(primary_keys) + list(fallback_keys)

        for key in primary_keys:
            if key in v_hparams:
                return v_hparams[key]
            # support nested patches.size (could be int or tuple/list)
            if (
                key == "patch_size"
                and "patches" in v_hparams
                and isinstance(v_hparams["patches"], dict)
            ):
                if "size" in v_hparams["patches"]:
                    sz = v_hparams["patches"]["size"]
                    # If it is a list/tuple like [16, 16], take first element
    ...
    fout.add_uint32("clip.vision.image_size", vparam("image_size", default=224))
    fout.add_uint32("clip.vision.patch_size", vparam("patch_size", default=16))
    fout.add_uint32(
        k(KEY_EMBEDDING_LENGTH, VISION),
        vparam(["hidden_size", "width", "d_model"], default=768),
    )
    fout.add_uint32(
        k(KEY_FEED_FORWARD_LENGTH, VISION),
        vparam(
            ["intermediate_size", "mlp_ratio"],
            default=vparam(["hidden_size", "width", "d_model"], default=768) * 4,
        ),
    )
    fout.add_uint32("clip.vision.projection_dim", visual_projection_dim)
    fout.add_uint32(
        k(KEY_ATTENTION_HEAD_COUNT, VISION),
        vparam(["num_attention_heads", "n_heads"], default=12),
    )
    fout.add_float32(
        k(KEY_ATTENTION_LAYERNORM_EPS, VISION), vparam("layer_norm_eps", default=1e-5)
    )
    if feature_layers:
        block_count = max(feature_layers)
    else:
        total_layers = vparam(["num_hidden_layers", "n_layers"], default=12)
        block_count = total_layers - 1 if has_llava_projector else total_layers
...
activation_name = vparam(
    ["hidden_act", "activation", "activation_fn", "act_fn"], default="quick_gelu"
)
use_gelu = activation_name == "gelu"
...
# ---------------------------------------------------------------------------
# Finalize global metadata flags now that we know whether the text encoder is
# actually present. These must be added before writing the header.
# ---------------------------------------------------------------------------
