...
// logging
//
static int                   g_obj_count_for_debug_ctx = 0;
static struct ggml_context * g_debug_ctx_ptr           = NULL;
static size_t                g_debug_ctx_expected_size = 0;
...
static void ggml_vec_dot_f32(int n, float * GGML_RESTRICT s, size_t bs, const float * GGML_RESTRICT x, size_t bx,
                             const float * GGML_RESTRICT y, size_t by, int nrc);
static void ggml_vec_dot_f16(int n, float * GGML_RESTRICT s, size_t bs, ggml_fp16_t * GGML_RESTRICT x, size_t bx,
                             ggml_fp16_t * GGML_RESTRICT y, size_t by, int nrc);
static void ggml_vec_dot_bf16(int n, float * GGML_RESTRICT s, size_t bs, ggml_bf16_t * GGML_RESTRICT x, size_t bx,
...
static const char * GGML_UNARY_OP_NAME[GGML_UNARY_OP_COUNT] = {
    "ABS",     "SGN",  "NEG",        "STEP", "TANH",      "ELU",         "RELU",
    "SIGMOID", "GELU", "GELU_QUICK", "SILU", "HARDSWISH", "HARDSIGMOID", "EXP",
};
...
struct ggml_context * ggml_init(struct ggml_init_params params) {
    ...
    // DEBUG: If this is the context we are interested in (ctx_data from clip.cpp)
    // For SigLIP SO400M (887 base, 27 layers for V and T, +1 safety) => (887 + 54 + 54 + 1) * 368 = 996 * 368 = 366528
    const size_t EXPECTED_CTX_DATA_SIZE_FOR_SIGLIP_SO400M = 366528;     // Update this if your slot calculation changes
    if (params.no_alloc == true &&
        params.mem_size == EXPECTED_CTX_DATA_SIZE_FOR_SIGLIP_SO400M) {  // Target specific size
        g_debug_ctx_ptr           = NULL;                               // Will be set after ctx is MALLOC'd
        g_debug_ctx_expected_size = params.mem_size;
        fprintf(stderr, "[GGML_INIT_DEBUG] Targeting ctx_data with expected size %zu for object counting\n",
                g_debug_ctx_expected_size);
    } else if (params.no_alloc == true) {
        // Fallback for general no_alloc contexts if size doesn't match, or for broader debugging
        fprintf(stderr,
                "[GGML_INIT_DEBUG] Non-targeted no_alloc context, size %zu. Not enabling detailed object counting for "
                "this one.\n",
                params.mem_size);
    }
    ...
    const size_t mem_size_final_for_ctx =
        params.mem_buffer ? params.mem_size : GGML_PAD(params.mem_size, GGML_MEM_ALIGN);
    // If this is the context we're tracking, set the global pointer
    *ctx = (struct ggml_context) {
        /*.mem_size           =*/mem_size_final_for_ctx,
        /*.mem_buffer         =*/params.mem_buffer ? params.mem_buffer : ggml_aligned_malloc(mem_size_final_for_ctx),
    ...
    if (params.no_alloc == true && ctx->mem_size == g_debug_ctx_expected_size && g_debug_ctx_ptr == NULL) {
        g_debug_ctx_ptr           = ctx;  // Set after ctx is MALLOC'd
        g_obj_count_for_debug_ctx = 0;
        // fprintf(stderr, "[GGML_DEBUG_COUNTER] Initialized counter for targeted ctx_data %p with final size %zu\n",
        //         (void *) ctx, ctx->mem_size);
    }
...
static struct ggml_object * ggml_new_object(struct ggml_context * ctx, enum ggml_object_type type, size_t size) {
    ...
    // if (ctx == g_debug_ctx_ptr) {
    //     fprintf(stderr,
    //             "[GGML_DEBUG_COUNTER] Attempting object %d in ctx %p. Needed for this obj (payload+GGML_OBJ_SIZE): "
    //             "%zu. Space left before this obj: %zu. cur_end: %zu\n",
    //             g_obj_count_for_debug_ctx + 1, (void *) ctx, size_needed + GGML_OBJECT_SIZE, ctx->mem_size - cur_end,
    //             cur_end);
    // }
    ...
    *obj_new = (struct ggml_object) {
    ...
    if (ctx == g_debug_ctx_ptr) {
        g_obj_count_for_debug_ctx++;
        // Name will be set by caller if it's a tensor
    }
    ...
    //     g_obj_count_for_debug_ctx++;
    //     fprintf(stderr,
    //             "[GGML_DEBUG_COUNTER] Context %p, Object %d allocated (type %d). cur_end now: %zu, obj_new->offs: %zu, "
    //             "obj_new->size: %zu\n",
    //             (void *) ctx, g_obj_count_for_debug_ctx, (int) type, cur_end, obj_new->offs, obj_new->size);
    //     if (type == GGML_OBJECT_TYPE_TENSOR) {
    //         // Temporarily cast to get name if we are about to create a tensor object
    //         // This is risky as result is not fully formed yet.
    //         // Better to do this after ggml_new_tensor_impl fully forms 'result'
    //     }
    // }
...
static struct ggml_tensor * ggml_new_tensor_impl(struct ggml_context * ctx, enum ggml_type type, int n_dims,
    ...
    //GGML_ASSERT(obj_new);
    if (!obj_new) {  // If ggml_new_object returned NULL (due to assert or otherwise)
        // The GGML_ASSERT inside ggml_new_object would have already aborted in debug mode.
        // In release, it would return NULL.
        fprintf(stderr, "%s: ggml_new_object failed for tensor (type %d)\n", __func__, (int) type);
        return NULL;
    }
    ...
    *result = (struct ggml_tensor) {
    ...
    // Set name before potential debug print
    // Placeholder, actual name set by ggml_set_name or ggml_format_name by caller
    // For debug, we can at least see if it's a view or base.
    snprintf(result->name, sizeof(result->name), "tensor_%d", ctx->n_objects);
    ...
    // if (ctx == g_debug_ctx_ptr && obj_new->type == GGML_OBJECT_TYPE_TENSOR) {
    //     fprintf(stderr, "[GGML_DEBUG_TENSOR_CREATED] Context %p, Tensor Object %d created: name='%s'\n", (void *) ctx,
    //             g_obj_count_for_debug_ctx, result->name);
    // }
    ctx->n_objects++;
...
void ggml_unravel_index(const struct ggml_tensor * tensor, int64_t i, int64_t * i0, int64_t * i1, int64_t * i2,
                        int64_t * i3) {
...
static struct ggml_tensor * ggml_add_cast_impl(struct ggml_context * ctx, struct ggml_tensor * a,
...
struct ggml_tensor * ggml_add_cast(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                   enum ggml_type type) {
...
static struct ggml_tensor * ggml_acc_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                          size_t nb1, size_t nb2, size_t nb3, size_t offset, bool inplace) {
...
struct ggml_tensor * ggml_leaky_relu(struct ggml_context * ctx, struct ggml_tensor * a, float negative_slope,
                                     bool inplace) {
    struct ggml_tensor * result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
static struct ggml_tensor * ggml_l2_norm_impl(struct ggml_context * ctx, struct ggml_tensor * a, float eps,
                                              bool inplace) {
    struct ggml_tensor * result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
*/
struct ggml_tensor * ggml_mul_mat_id(struct ggml_context * ctx, struct ggml_tensor * as, struct ggml_tensor * b,
...
static struct ggml_tensor * ggml_set_impl(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                          size_t nb1, size_t nb2, size_t nb3, size_t offset, bool inplace) {
...
GGML_API struct ggml_tensor * ggml_cont_2d(struct ggml_context * ctx, struct ggml_tensor * a, int64_t ne0,
                                           int64_t ne1) {
...
struct ggml_tensor * ggml_cont_4d(struct ggml_context * ctx, struct ggml_tensor * a, int64_t ne0, int64_t ne1,
                                  int64_t ne2, int64_t ne3) {
...
struct ggml_tensor * ggml_view_3d(struct ggml_context * ctx, struct ggml_tensor * a, int64_t ne0, int64_t ne1,
                                  int64_t ne2, size_t nb1, size_t nb2, size_t offset) {
    const int64_t ne[3] = { ne0, ne1, ne2 };
...
struct ggml_tensor * ggml_view_4d(struct ggml_context * ctx, struct ggml_tensor * a, int64_t ne0, int64_t ne1,
                                  int64_t ne2, int64_t ne3, size_t nb1, size_t nb2, size_t nb3, size_t offset) {
    const int64_t ne[4] = { ne0, ne1, ne2, ne3 };
...
static struct ggml_tensor * ggml_soft_max_impl(struct ggml_context * ctx, struct ggml_tensor * a,
                                               struct ggml_tensor * mask, float scale, float max_bias, bool inplace) {
    GGML_ASSERT(ggml_is_contiguous(a));
...
struct ggml_tensor * ggml_soft_max_ext(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * mask,
...
static struct ggml_tensor * ggml_soft_max_ext_back_impl(struct ggml_context * ctx, struct ggml_tensor * a,
...
struct ggml_tensor * ggml_soft_max_ext_back(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                            float scale, float max_bias) {
...
struct ggml_tensor * ggml_rope(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, int n_dims,
                               int mode) {
    return ggml_rope_impl(ctx, a, b, NULL, n_dims, mode, 0, 10000.0f, 1.0f, 0.0f, 1.0f, 0.0f, 0.0f, false);
}

struct ggml_tensor * ggml_rope_multi(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                     struct ggml_tensor * c, int n_dims, int sections[4], int mode, int n_ctx_orig,
                                     float freq_base, float freq_scale, float ext_factor, float attn_factor,
...
struct ggml_tensor * ggml_rope_inplace(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                       int n_dims, int mode) {
    return ggml_rope_impl(ctx, a, b, NULL, n_dims, mode, 0, 10000.0f, 1.0f, 0.0f, 1.0f, 0.0f, 0.0f, true);
}

struct ggml_tensor * ggml_rope_ext(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                   struct ggml_tensor * c, int n_dims, int mode, int n_ctx_orig, float freq_base,
                                   float freq_scale, float ext_factor, float attn_factor, float beta_fast,
                                   float beta_slow) {
    return ggml_rope_impl(ctx, a, b, c, n_dims, mode, n_ctx_orig, freq_base, freq_scale, ext_factor, attn_factor,
                          beta_fast, beta_slow, false);
}

struct ggml_tensor * ggml_rope_ext_inplace(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                           struct ggml_tensor * c, int n_dims, int mode, int n_ctx_orig,
                                           float freq_base, float freq_scale, float ext_factor, float attn_factor,
                                           float beta_fast, float beta_slow) {
    return ggml_rope_impl(ctx, a, b, c, n_dims, mode, n_ctx_orig, freq_base, freq_scale, ext_factor, attn_factor,
...
struct ggml_tensor * ggml_rope_custom(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
    ...
    return ggml_rope_impl(ctx, a, b, NULL, n_dims, mode, n_ctx_orig, freq_base, freq_scale, ext_factor, attn_factor,
                          beta_fast, beta_slow, false);
}
...
struct ggml_tensor * ggml_rope_ext_back(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                        struct ggml_tensor * c, int n_dims, int mode, int n_ctx_orig, float freq_base,
                                        float freq_scale, float ext_factor, float attn_factor, float beta_fast,
                                        float beta_slow) {
...
struct ggml_tensor * ggml_rope_multi_back(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                          struct ggml_tensor * c, int n_dims, int sections[4], int mode, int n_ctx_orig,
                                          float freq_base, float freq_scale, float ext_factor, float attn_factor,
                                          float beta_fast, float beta_slow) {
...
struct ggml_tensor * ggml_im2col(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, int s0,
                                 int s1, int p0, int p1, int d0, int d1, bool is_2D, enum ggml_type dst_type) {
    if (is_2D) {
...
struct ggml_tensor * ggml_conv_1d(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, int s0,
                                  int p0, int d0) {
...
struct ggml_tensor * ggml_conv_1d_dw(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, int s0,
                                     int p0, int d0) {
...
GGML_API struct ggml_tensor * ggml_conv_transpose_1d(struct ggml_context * ctx, struct ggml_tensor * a,
...
// b: [N, IC, IH, IW]
// result: [N, OC, OH, OW]
struct ggml_tensor * ggml_conv_2d(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, int s0,
...
struct ggml_tensor * ggml_conv_2d_dw(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b, int s0,
                                     int s1, int p0, int p1, int d0, int d1) {
...
struct ggml_tensor * ggml_conv_2d_dw_direct(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b,
                                            int stride0, int stride1, int pad0, int pad1, int dilation0,
                                            int dilation1) {
...
struct ggml_tensor * ggml_pool_1d(struct ggml_context * ctx, struct ggml_tensor * a, enum ggml_op_pool op, int k0,
                                  int s0, int p0) {
    const int64_t ne[4] = {
...
struct ggml_tensor * ggml_pool_2d(struct ggml_context * ctx, struct ggml_tensor * a, enum ggml_op_pool op, int k0,
                                  int k1, int s0, int s1, float p0, float p1) {
    struct ggml_tensor * result;
    const int64_t        ne[4] = {
        ggml_calc_pool_output_size(a->ne[0], k0, s0, p0),
...
struct ggml_tensor * ggml_pool_2d_back(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * af,
...
static struct ggml_tensor * ggml_upscale_impl(struct ggml_context * ctx, struct ggml_tensor * a, int ne0, int ne1,
                                              int ne2, int ne3, enum ggml_scale_mode mode) {
...
struct ggml_tensor * ggml_upscale_ext(struct ggml_context * ctx, struct ggml_tensor * a, int ne0, int ne1, int ne2,
                                      int ne3, enum ggml_scale_mode mode) {
...
struct ggml_tensor * ggml_pad(struct ggml_context * ctx, struct ggml_tensor * a, int p0, int p1, int p2, int p3) {
    struct ggml_tensor * result =
        ggml_new_tensor_4d(ctx, a->type, a->ne[0] + p0, a->ne[1] + p1, a->ne[2] + p2, a->ne[3] + p3);
...
struct ggml_tensor * ggml_timestep_embedding(struct ggml_context * ctx, struct ggml_tensor * timesteps, int dim,
                                             int max_period) {
    int actual_dim = dim;
    if (dim % 2 != 0) {
        actual_dim = dim + 1;
...
struct ggml_tensor * ggml_flash_attn_ext(struct ggml_context * ctx, struct ggml_tensor * q, struct ggml_tensor * k,
                                         struct ggml_tensor * v, struct ggml_tensor * mask, float scale, float max_bias,
                                         float logit_softcap) {
...
static struct ggml_tensor * ggml_add_rel_pos_impl(struct ggml_context * ctx, struct ggml_tensor * a,
                                                  struct ggml_tensor * pw, struct ggml_tensor * ph, bool inplace) {
...
struct ggml_tensor * ggml_rwkv_wkv7(struct ggml_context * ctx, struct ggml_tensor * r, struct ggml_tensor * w,
                                    struct ggml_tensor * k, struct ggml_tensor * v, struct ggml_tensor * a,
                                    struct ggml_tensor * b, struct ggml_tensor * state) {
    GGML_ASSERT(ggml_is_contiguous(r));
...
static struct ggml_tensor * ggml_map_custom2_impl(struct ggml_context * ctx, struct ggml_tensor * a,
                                                  struct ggml_tensor * b, const ggml_custom2_op_t fun, int n_tasks,
...
struct ggml_tensor * ggml_custom_4d(struct ggml_context * ctx, enum ggml_type type, int64_t ne0, int64_t ne1,
                                    int64_t ne2, int64_t ne3, struct ggml_tensor ** args, int n_args,
...
struct ggml_tensor * ggml_custom_inplace(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor ** args,
                                         int n_args, ggml_custom_op_t fun, int n_tasks, void * userdata) {
...
struct ggml_tensor * ggml_opt_step_adamw(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * grad,
...
static void ggml_add_or_set(struct ggml_context * ctx, struct ggml_cgraph * cgraph, size_t isrc,
...
static void ggml_acc_or_set(struct ggml_context * ctx, struct ggml_cgraph * cgraph, size_t isrc,
                            struct ggml_tensor * tensor, const size_t nb1, const size_t nb2, const size_t nb3,
                            const size_t offset) {
    struct ggml_tensor * src = cgraph->visited_hash_set.keys[isrc];
    GGML_ASSERT(src);
    if (cgraph->grads[isrc]) {
...
static void ggml_add1_or_set(struct ggml_context * ctx, struct ggml_cgraph * cgraph, size_t isrc,
                             struct ggml_tensor * tensor) {
    struct ggml_tensor * src = cgraph->visited_hash_set.keys[isrc];
    GGML_ASSERT(src);
    if (cgraph->grads[isrc]) {
...
static void ggml_sub_or_set(struct ggml_context * ctx, struct ggml_cgraph * cgraph, size_t isrc,
                            struct ggml_tensor * tensor) {
    struct ggml_tensor * src = cgraph->visited_hash_set.keys[isrc];
    GGML_ASSERT(src);
    if (cgraph->grads[isrc]) {
...
static void ggml_compute_backward(struct ggml_context * ctx, struct ggml_cgraph * cgraph, int i,
...
size_t ggml_quantize_chunk(enum ggml_type type, const float * src, void * dst, int64_t start, int64_t nrows,
